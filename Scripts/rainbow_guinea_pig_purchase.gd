...
func _ready():
	...
	rainbow_pig_purchased.connect(get_parent().get_parent().get_node("Poop_Upgrades_Shop").get_node("ScrollContainer").get_node("VBoxContainer").get_node("Poop_Speed_Purchase")._new_pig_purchased.bind())
	rainbow_pig_purchased.connect(get_parent().get_parent().get_node("Garden")._update_guinea_dictionary.bind())
